...
Three-phase transformer training pipeline:
    ...
    Phase-2 : pretrain_transformer()  →  checkpoints/phase2/ckpt_best.pt
              (bidirectional MLM on the EMREncoder backbone)
    Phase-3 : finetune_transformer()  →  checkpoints/phase3/ckpt_best.pt
              (per-outcome risk + time TaskHeads on top of the encoder)
...
Contract:
  ...
  • All three phases use the natural distribution — MLM does not need rare-
    outcome rebalancing; Phase-3 leans on BCE pos_weight instead.
...
def run_training(reset_phase23=True):
    ...
    # Phase 2 — bidirectional encoder MLM pre-training (natural distribution).
    # MLM masking lets the model see every patient at the natural prevalence —
    # rare-outcome rebalancing is the job of Phase-3 pos_weight, not Phase-2.
    model_p2 = EMREncoder(cfg=MODEL_CONFIG, embedder=embedder)
    ...
    # Phase 3 — attach task heads on top of best Phase-2 ckpt, fine-tune at
    # phase3_backbone_lr_factor backbone LR + full head LR.
...
if __name__ == "__main__":
    run_training()
    # Release training references; final eval is driven by api.py + evaluation.py.
    gc.collect()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    print("[Train] Done. Run api.py to score the held-out test split.")
