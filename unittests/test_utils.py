...
import pytest
import intervene_enc.utils as utils_module
...
from intervene_enc.schedulers import LambdaScheduleController, linear_schedule as sched_linear_schedule
from intervene_enc.dataset import EMRTokenizer
from intervene_enc.utils import compute_soft_outcome_labels
...
def test_unified_lambda_schedule_controller_phase1_alias_and_immediate_activation():
    ...
    # Warmup gate = ramp_end + 1 calibration-delay offset (=> 2).
    assert controller.current_warmup_end_epoch() == 2
...
def test_precalibration_warmup_gate_at_start_epoch_not_plus_one():
    ...
    # aux start_epoch = 1 + 3 = 4; warmup gate = ramp_end + 1 = 5 (calibration-delay offset).
    assert controller.current_warmup_end_epoch() == 5
    ...
    # Warmup gate = ramp_end + 1 calibration-delay offset = 5.
    assert controller.current_warmup_end_epoch() == 5
...
def mini_tokenizer():
    ...
    toks = [
        "[PAD]", "[MASK]", "[CTX]", "[NULL]",
        # Hierarchical MLM mask tokens (required by EMRTokenizer __init__).
...
def test_apply_cbm_visual_and_assert(mini_tokenizer):
    ...
    if eligible_min is not None and other_min is not None:
        ...
        out3 = apply_cbm(
            batch3.copy(),
            tokenizer=tk, forbid_ids=minimal_forbid, luts=luts, p=1.0
        )
