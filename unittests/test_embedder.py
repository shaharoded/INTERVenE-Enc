...
def mini_tokenizer():
    # Minimal tokenizer for testing embedder.  Includes the hierarchical MLM
    # mask tokens that the BERT-pivot tokenizer requires.
    toks = ["[PAD]", "[MASK]", "[CTX]", "[NULL]",
            "[MASK_INTERVAL_START]", "[MASK_INTERVAL_END]",
            "A_START", "A_END"]
    token2id = {t:i for i,t in enumerate(toks)}
    rawconcept2id = {tok: i for i, tok in enumerate(
        ["[PAD]", "[MASK]", "[CTX]", "[NULL]",
         "[MASK_INTERVAL_START]", "[MASK_INTERVAL_END]", "A"]
    )}
    concept2id    = dict(rawconcept2id)
    value2id      = dict(rawconcept2id)
