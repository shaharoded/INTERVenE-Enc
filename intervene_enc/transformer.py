...
def finetune_transformer(model, train_dl, val_dl, resume=True,
    ...
    # (DEATH, SEVERE_HYPOGLYCEMIA) drive a larger share of the gradient.
    # γ = 0 falls back to plain BCEWithLogitsLoss.
    focal_gamma = float(training_settings.get("phase3_focal_gamma", 0.0))
    ...
    def risk_criterion(logits, targets):
        ...
        if focal_gamma <= 0.0:
            return bce.mean()
        # p_t = predicted probability of the correct class (handles 0/1 targets).
        with torch.no_grad():
            p = torch.sigmoid(logits.detach())
        p_t = targets * p + (1.0 - targets) * (1.0 - p)
        focal_w = (1.0 - p_t).clamp(min=0.0).pow(focal_gamma)
