...
def length_of_stay_mae(predictions, gt_episodes, release_token=RELEASE_TOKEN):
    ...
    Method:  For each patient with a GT release:
    ...
    Returns:
        dict: mae_hours, median_hours, p90_hours, n_patients,
...
def time_head_mae(predictions, gt_episodes, outcome_names):
    """
    Purpose: Per-outcome time-head MAE among positive patients only.
    Method:  For each (patient, outcome) where the outcome occurred in GT,
             compute |T_<outcome> − nearest GT occurrence|. Mean over positives.
    ...
    for name in outcome_names:
        ...
        for pid in predictions.index:
            ...
            errs.append(min(abs(pred_t - t_gt) for t_gt in episodes))
        rows.append({
            "outcome":    name,
    ...
    if len(time_mae_table):
        ...
        for outcome, row in time_mae_table.iterrows():
            if not np.isnan(row["mae_hours"]):
                print(f"  {outcome:<45} MAE={row['mae_hours']:.2f}h  n_pos={int(row['n_patients'])}")
